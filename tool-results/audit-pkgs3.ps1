...
function G($p){ return (Send GET $p)|J }
function Msg($s){ $o=(J $s); if($o.error){return $o.error} else {return ($s -split '\|',2)[0]} }
...
Write-Host ("ADD QUESTIONS: " + $add.Split('|')[0] + " -> " + (Msg $add))
# Try delete via various actions
foreach($a in @("delete","delete-package","remove")){
  $d = Send DELETE "mcq-exam-packages?action=$a&id=$pkgId"
  Write-Host ("PKG DELETE action=$a : " + $d.Split('|')[0] + " -> " + (Msg $d))
}
...
Write-Host ("CQ PKG CREATE: " + $cqp.Split('|')[0] + " -> " + (Msg $cqp))
$u = G "users?limit=1"
