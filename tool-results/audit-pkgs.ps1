...
function Id($s,$k){ $o=(J $s|ConvertFrom-Json); $o.$k }

# valid ids
$subj = (Send GET "subjects?classId=ssc&limit=1"|ConvertFrom-Json)
$sid = $subj.data[0].id
$chap = (Send GET "chapters?subjectId=$sid&limit=1"|ConvertFrom-Json)
$cid = $chap.data[0].id

# ---- MCQ exam package lifecycle ----
$cls = (Send GET "classes?limit=1"|ConvertFrom-Json); $clid=$cls.data[0].id
...
$pid = Id $p "data.id"
$set = Send POST "mcq-exam-packages" @{action="create-set"; packageId=$pid; title="AUDIT SET1"; scheduledDate="2026-08-01"; duration=30}
Write-Host "SET CREATE: $($set.Split('|')[0])"
$setid = Id $set "data.id"
# create an MCQ to add
...
$mid = Id $m "data.id"
$add = Send POST "mcq-exam-packages" @{action="add-questions"; setId=$setid; mcqIds=@($mid)}
Write-Host "ADD QUESTIONS: $($add.Split('|')[0])"
# list packages
Write-Host "PKG LIST: $((Send GET "mcq-exam-packages?action=list").Split('|')[0])"
# cleanup
Write-Host "PKG DELETE: $(Send DELETE "mcq-exam-packages?id=$pid")"
Write-Host "MCQ cleanup: $(Send DELETE "mcq?id=$mid")"

# ---- CQ exam package list (action) ----
...
# ---- USERS delete behavior (destructive cascade) ----
$u = Send GET "users?limit=1"
$udata = $u|ConvertFrom-Json
Write-Host "USERS list count: $($udata.data.Count); total: $($udata.pagination.total)"
# check bulk import module
